...
public class MyCollectionPresenter extends BasePresenter<MyCollectionContract.Model, MyCollectionContract.View> {
    ...
    public void getCollectionGoods(boolean pullToRefresh, boolean showLoading) {
        if (pullToRefresh) {
            page = CommonConstant.PAGE_INITIAL;
        }
        mModel.getCollectionGoods(page)
                .compose(new CommonTransformer<>(this, showLoading))
                .subscribe(new ErrorHandleSubscriber<Optional<CouponsCommodityBean>>(mErrorHandler) {
                    @Override
                    public void onNext(Optional<CouponsCommodityBean> couponsCommodityBeanOptional) {
                        page++;
                        mRootView.onGetCollectionGoodsSuccess(couponsCommodityBeanOptional.isEmpty() ? null : couponsCommodityBeanOptional.get().getList());
                    }

                    @Override
                    public void onError(Throwable t) {
                        super.onError(t);
                        mRootView.onGetCollectionGoodsFailed();
                    }
                });
    }
