...
public class WithdrawPresenter extends BasePresenter<WithdrawContract.Model, WithdrawContract.View> {
    ...
    public void getBalance() {
        mModel.getBalance()
                ...
                .subscribe(new ErrorHandleSubscriber<Optional<WithDrawBean>>(mErrorHandler) {
                    @Override
                    public void onNext(Optional<WithDrawBean> withDrawBeanOptional) {
                        mRootView.onBalanceSuccess(withDrawBeanOptional.isEmpty() ? null : withDrawBeanOptional.get().getTotalAccount());
                    }
                });
    }

    public void withdraw(String money, String mobile, String verifyCode) {
        mModel.withdraw(money, mobile, verifyCode)
                .compose(new CommonTransformer<>(this, false))
