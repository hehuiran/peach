...
public interface MessageContract {
    ...
    interface View extends IView {
        void onGetMessageListSuccess(List<MessageBean.DataBean> list);

        void onGetMessageListFailed();

        void onDeleteMessageSuccess(int position);

        void onDeleteMessageFailed(int position);
    }

    //Model层定义接口,外部只需关心Model返回的数据,无需关心内部细节,即是否使用缓存
    interface Model extends IModel {
        Observable<MessageBean> getMessageList(int page);

        Observable<BasicResponse<String>> deleteMessage(String id);
    }
}
